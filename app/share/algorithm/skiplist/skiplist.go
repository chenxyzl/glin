...
func New[V Valuer](cmp Comparator[V]) *SkipList[V] {
	sl := &SkipList[V]{
		level:  1,
		length: 0,
		tail:   nil,
...
func (l *SkipList[V]) Insert(value V) *Node[V] {
...
func (l *SkipList[V]) Get(key uint64) (V, bool) {
	v, b := l.index[key]
	return v, b
}
...
// ForRange 遍历 true则继续, false则停止
func (l *SkipList[V]) ForRange(fun func(v V) bool, reverse ...bool) {
...
func (l *SkipList[V]) GetRange(start uint32, end uint32, reverse bool) []V {
	// var retKey []uint64
	// var retScore []uint64
	var out []V
	if start == 0 {
	...
	if start > end || start > uint32(l.length) {
		return out
	}
	if end > uint32(l.length) {
		end = uint32(l.length)
	}
	rangeLen := end - start + 1
	if reverse {
		node := l.GetNodeByRank(uint32(l.length) - start + 1)
