...
func GetCol[T model.IModel](mod T) *mongo.Collection {
	if mongoClient == nil {
		slog.Panicf("mongo client not connect")
	}
	return mongoClient.Database(config.Get().AConfig.Mongo.DbName).Collection(mod.CoName())
}

func GetColByStr(name string) *mongo.Collection {
	if mongoClient == nil {
		slog.Panicf("mongo client not connect")
	}
	return mongoClient.Database(config.Get().AConfig.Mongo.DbName).Collection(name)
}

func Connect() *mongo.Client {
	//
	url := config.Get().AConfig.Mongo.Url
